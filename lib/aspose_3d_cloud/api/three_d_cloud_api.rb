...
module AsposeThreeDCloud
  class ThreeDCloudApi
    ...
    # @option opts [String] :storage The storage type
    # @return [File]
    def post_triangulate_part(name, objectaddressingpath, newfilename, newformat, opts = {})
      @api_client.request_token_if_needed
      data, _status_code, _headers = post_triangulate_part_with_http_info(name, objectaddressingpath, newfilename, newformat, opts)
      rescue ApiError => error
        if error.code == 401
          @api_client.request_token_if_needed
                data, _status_code, _headers = post_triangulate_part_with_http_info(name, objectaddressingpath, newfilename, newformat, opts)
    ...
    # @option opts [String] :storage The storage type
    # @return [Array<(File, Fixnum, Hash)>] File data, response status code and response headers
    def post_triangulate_part_with_http_info(name, objectaddressingpath, newfilename, newformat, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: ThreeDCloudApi.post_triangulate_part ..."
      end
      # verify the required parameter 'name' is set
      if @api_client.config.client_side_validation && name.nil?
        fail ArgumentError, "Missing the required parameter 'name' when calling ThreeDCloudApi.post_triangulate_part"
      end
      # verify the required parameter 'objectaddressingpath' is set
      if @api_client.config.client_side_validation && objectaddressingpath.nil?
        fail ArgumentError, "Missing the required parameter 'objectaddressingpath' when calling ThreeDCloudApi.post_triangulate_part"
      end
      # verify the required parameter 'newfilename' is set
      if @api_client.config.client_side_validation && newfilename.nil?
        fail ArgumentError, "Missing the required parameter 'newfilename' when calling ThreeDCloudApi.post_triangulate_part"
      end
      # verify the required parameter 'newformat' is set
      if @api_client.config.client_side_validation && newformat.nil?
        fail ArgumentError, "Missing the required parameter 'newformat' when calling ThreeDCloudApi.post_triangulate_part"
      end
      # resource path
      local_var_path = "/3d/triangulate/part"
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path,
        ...
        :return_type => 'File')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ThreeDCloudApi#post_triangulate_part\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
