  ...
  class ApiClient
    ...
    def convert_to_type(data, return_type)
      ...
      when /\AHash\<String, (.+)\>\z/
        ...
        # models, e.g. Pet
        AsposeThreeDCloud.const_get(return_type).new.tap do |model|
    ...
    def download_file(response)
      ...
      f=File.new(File.join("D:","download-aspose-000.pdf"), "wb+")#b:binary ,must have,or will cause 0A -> 0A 0D issue in file and then open crash
      f.puts(response.body)
      f.close
      #tempfile can not save correctly,which always cause  0A -> 0A 0D issue in file
      # so I delete next code which is got by translation
=begin
      encoding = response.body.encoding   #'ASCII-8BIT'   #'Binary'
